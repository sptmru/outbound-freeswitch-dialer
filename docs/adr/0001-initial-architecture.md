...
The product needs an outbound dialer with a Web UI softphone, FreeSWITCH, ESL, Docker, Node.js, and TypeScript. It must support click-to-call, manual voicemail drop, prerecorded audio playback into an active call, agent release once playback starts, campaigns, CSV lead import, suppression checks, optional call recording, call logging, and SIP trunk integration.
...
- FreeSWITCH handles media, SIP profiles, bridge behavior, SIP trunking, and playback.
...
For click-to-call, the backend should originate the agent leg first, then originate the customer leg through the SIP trunk and bridge the two legs after both are ready.
...
For the SIP trunk provider, support both registration-based and IP-authenticated trunk configuration, selected through runtime configuration.
