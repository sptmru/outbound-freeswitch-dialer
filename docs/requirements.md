...
- Agent Desk's Next leads/Lead queue and Lead context show a Zoho CRM link only for a non-empty `lead_id` from CSV import or the optional **Zoho Lead ID** field in manual lead creation, opening `https://crm.zoho.com.au/crm/org7002688441/tab/Leads/{lead_id}` in a new tab. IDs remain strings and are available regardless of CSV column position or the eight-field display limit. Lead context shows the CRM link instead of the raw `lead_id`, independently of its five-field display limit.
