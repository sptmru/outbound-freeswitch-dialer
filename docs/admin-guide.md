...
- In **Add lead**, optionally enter **Zoho Lead ID** to link a manually created lead to its Zoho CRM profile. Leave it empty when no CRM profile is available. The same links appear in Next leads, Lead queue, and Lead context.
- Include an optional `lead_id` column to show a **Zoho CRM** link in the agent's lead queue. Keep IDs as text when preparing spreadsheets so long values such as `51445000042207511` remain exact. Blank values hide the link. The link uses `https://crm.zoho.com.au/crm/org7002688441/tab/Leads/{lead_id}`. Previously imported IDs work too; re-importing a duplicate phone number does not update an existing contact or add a missing ID.
