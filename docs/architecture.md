...
5. The browser answers its internal SIP leg. FreeSWITCH originates the customer leg through the configured trunk and passes provider early media through. After a short grace period, the API refreshes and polls the customer leg's inbound RTP media-packet counter; it injects local ringback toward the agent only while that counter is zero, then stops it as soon as callee RTP appears or the call answers/ends.
