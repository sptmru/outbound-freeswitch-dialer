...
- Conditional Zoho CRM profile links in Next leads/Lead queue and Lead context using `lead_id` strings from CSV import or the optional Zoho Lead ID field in Add lead, exposed separately from the limited list of display fields.
...
- Campaign create/edit/status/archive, confirmed lead reset with active-call protection, streaming ZIP export of playable campaign recordings with manifest/audit coverage, plus manual-dial, recording, early-media AVMD, and automatic next-lead flags.
