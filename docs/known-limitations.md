...
- Per-leg RTP counters, MOS, jitter, codecs, and gateway/profile are captured from FreeSWITCH at `CHANNEL_HANGUP_COMPLETE`. Browser summaries add playout loss/concealment/jitter-buffer/RTT evidence and applied microphone settings, but they still do not prove that speech was intelligible, that the selected microphone contained speech, or what a PSTN participant heard. Browser submission is best-effort and can be absent after an abrupt tab/process/network failure. “Suspected one-way” is intentionally a conservative diagnostic flag.
